...
export async function PATCH(request, { params }) {
  ...
  return json({ album: modifierAlbum(params.id, champs) });
}
...
export async function POST(request, { params }) {
  ...
  const morceau = getTrackRow(trackId || "");
  ...
  rattacherTitre(morceau.id, detacher ? null : params.id, detacher ? null : trackNo ?? null);
  return json({ ok: true });
}
